...
function cloneDefaultData() {
  return {
    ...
    users: [],
    userSettings: {
      allowedModels: [],       // [] = all allowed, non-empty = whitelist
...
function ensureDbShape(data) {
  ...
  for (const [key, defaultValue] of Object.entries(defaults)) {
    ...
    // Migrate userSettings defaults
    if (key === "userSettings" && next.userSettings && typeof next.userSettings === "object") {
      if (next.userSettings.allowedModels === undefined) {
        next.userSettings.allowedModels = [];
        changed = true;
      }
      if (next.userSettings.registrationEnabled === undefined) {
        next.userSettings.registrationEnabled = true;
        changed = true;
      }
    }
  }
...
export async function updateUser(id, data) {
  ...
  await safeWrite(db);
  const { passwordHash: _, ...safe } = db.data.users[index];
  return safe;
}
...
export async function getUserSettings() {
  const db = await getDb();
  return db.data.userSettings || { allowedModels: [], registrationEnabled: true };
}

export async function updateUserSettings(updates) {
  const db = await getDb();
  if (!db.data.userSettings) {
    db.data.userSettings = { allowedModels: [], registrationEnabled: true };
  }
  db.data.userSettings = { ...db.data.userSettings, ...updates };
  await safeWrite(db);
  return db.data.userSettings;
}
